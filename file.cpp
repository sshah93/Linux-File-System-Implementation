...
{
	unsigned int add = block_id * block_size;
	block_addresses.insert(pair<unsigned int, unsigned int>(add, block_id));
}

const void file::printInfo()
{
	cout << "file: " << name << "size: " << size << " total blocks used: " << block_addresses.size() << endl;
